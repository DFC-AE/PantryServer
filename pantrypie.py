...
#                raise RuntimeError("OPENWEATHER_KEY environment variable not set.")
    ...
    def create_home_screen(self, item=None):
        ...
        self.clock_label = tk.Label(self.root, font=APP_FONT_TITLE_BOLD,
                                    background=self.bg_color, foreground=fg_color)
        ...
        self.create_random_recipe_panel(middle_frame)
        self.create_conversion_table_panel(middle_frame)
...
#           self.set_btn.pack(side=tk.LEFT)
    ...
    def create_expiring_soon_panel(self, parent):
        ...
        list_frame = tk.Frame(panel, bg=self.bg_color)
        list_frame.pack(fill=tk.BOTH, expand=True)

        canvas = Canvas(list_frame, bg=self.bg_color, highlightthickness=0)
    ...
    def populate_expiring_items(self):
        ...
        for item in self.items:
            ...
            # Only include items expiring in the next 3 days (including today)
            if 0 <= days_left <= 3:
        ...
        if hasattr(self, "expiring_frame") and self.expiring_frame.winfo_exists():
            ...
            if not expiring_items:
                tk.Label(self.expiring_frame, text="No items expiring soon.",
                        font=APP_FONT, bg=self.bg_color, fg="#2E2E2E").pack(pady=10)
            else:
                for name, days_left, exp_dt in expiring_items:
                    ...
                    label = tk.Label(self.expiring_frame,
                                    ...
                                    font=APP_FONT, bg=self.bg_color, fg=color, anchor="w", justify="left")
    ...
    def create_random_recipe_panel(self, parent):
        ...
        # Recipe image
        self.recipe_image_label = tk.Label(panel, bg=self.bg_color)
...
#        canvas.configure(yscrollcommand=scrollbar.set)
        ...
        try:
            ...
            self.recipe_full_image_label = tk.Label(content_frame, image=photo, bg=self.bg_color)
        ...
        ## Meal name
#        tk.Label(scrollable_frame, text=meal["strMeal"], font=APP_FONT_BOLD, bg=self.bg_color).pack(pady=10)

        ## Category and rating (simulated rating)
        #rating = "4/5"
#        rating = random.uniform(3.5, 5.0)
#        tk.Label(scrollable_frame, text=f"Category: {meal['strCategory']}   {rating}",
#                 font=APP_FONT, bg=self.bg_color).pack(pady=5)

#        tk.Label(scrollable_frame, text="Ingredients:", font=APP_FONT, bg=self.bg_color).pack(pady=5)
#        for i in range(1, 21):
#            ingredient = meal.get(f"strIngredient{i}")
#            measure = meal.get(f"strMeasure{i}")
#            if ingredient and ingredient.strip():
#                tk.Label(scrollable_frame, text=f" {ingredient} - {measure}",
#                         font=APP_FONT, bg=self.bg_color, anchor="w", justify="left").pack(fill=tk.X, padx=20)

        # --- Ingredients (full-width) ---
        ingredients_frame = tk.Frame(scrollable_frame, bg=self.bg_color)
        ingredients_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=5)

        tk.Label(ingredients_frame, text="Ingredients:", font=APP_FONT, bg=self.bg_color, anchor="w").pack(anchor="w")
        for i in range(1, 21):
            ingredient = meal.get(f"strIngredient{i}")
            measure = meal.get(f"strMeasure{i}")
            if ingredient and ingredient.strip():
                tk.Label(ingredients_frame, text=f"{ingredient} - {measure}",
                         font=APP_FONT, bg=self.bg_color, anchor="w", justify="left").pack(fill="x", padx=(5,0), pady=1)

        # --- Instructions (full-width) ---
        instructions_frame = tk.Frame(scrollable_frame, bg=self.bg_color)
        instructions_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=(10, 5))

        tk.Label(instructions_frame, text="Instructions:", font=APP_FONT, bg=self.bg_color, anchor="w").pack(anchor="w")
        tk.Label(instructions_frame, text=meal.get("strInstructions", ""),
                 wraplength=1000, justify="left", font=APP_FONT, bg=self.bg_color).pack(anchor="w", pady=5)
        ...
        # Instructions
#        tk.Label(scrollable_frame, text="Instructions:", font=APP_FONT, bg=self.bg_color).pack(pady=(10, 5))
#        tk.Label(scrollable_frame, text=meal["strInstructions"], wraplength=700, justify="left",
#                 font=APP_FONT, bg=self.bg_color).pack(padx=20, pady=5)
...
#        tk.Button(button_frame, text="Print", command=lambda: self.print_recipe(meal)).pack(side=tk.LEFT, padx=5)
    ...
    def save_settings_to_file(self):
        config = {
            ...
            "bg_color": self.bg_color  # Add this line
        }
        try:
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r") as f:
                    ...
                    self.bg_color = config.get("bg_color", "white")  # Add this line
            except Exception as e:
...
#        scrollbar.pack(side="bottom", fill="x")
        ...
        tk.Label(frame, text="Dark Mode:", font=(self.current_font, 14), bg=self.bg_color).grid(row=3, column=0, sticky="w")
        self.dark_mode_var = tk.BooleanVar(value=self.dark_mode)
        dark_toggle = tk.Checkbutton(frame, variable=self.dark_mode_var, command=self.preview_settings, bg=self.bg_color)
...
#        scroll_frame = tk.Frame(canvas, bg=self.bg_color)
        ...
        for item in self.items:
            ...
            frame = tk.Frame(scroll_frame, bg=self.bg_color, height=80, width=400)
            ...
            label = tk.Label(frame, text=text, bg=color, fg="#2E2E2E", font=APP_FONT, width=30, height=3, anchor="w", justify="left")
            label.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)
            # Make the delete button bigger
            tk.Button(frame, text="Delete", width=10, height=3, command=lambda i=item: self.delete_item(i)).pack(side=tk.RIGHT, padx=10, pady=10)
    ...
    def show_detail_view(self, item):
        ...
        if isinstance(item, dict):
            ...
            exp_date = getattr(item, "expiration_date", "")
            category = getattr(item, "category", "")
            barcode = getattr(item, "barcode", "")
            nutrition_info = getattr(item, "nutrition_info", {})
        ...
        if hasattr(item, "days_until_expired"):
            days = item.days_until_expired()
        elif isinstance(exp_date_obj, datetime):
            days = (exp_date_obj - datetime.now()).days + 1
        else:
            days = "?"

        detail_frame = tk.Frame(main_frame, bg=self.bg_color)
        detail_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=20, pady=20)

        detail_text = (
            f"Item: {name}\n"
            f"Expiration: {exp_date_obj.strftime('%Y-%m-%d') if isinstance(exp_date_obj, datetime) else exp_date}\n"
            f"Category: {category}\n"
            f"Barcode: {barcode}\n"
            f"Days Left: {check_dates(days) if isinstance(days, int) and days > 0 else 0}"
        )
        label = tk.Label(detail_frame, text=detail_text, font=APP_FONT, justify="left", bg=self.bg_color)
        label.pack(pady=30)

        def edit_item_fields():
            new_name = simpledialog.askstring("Edit Name", "Enter new name:", initialvalue=name)
            new_category = simpledialog.askstring("Edit Category", "Enter new category:", initialvalue=category)
            new_barcode = simpledialog.askstring("Edit Barcode", "Enter new barcode:", initialvalue=barcode)
            new_exp_date = simpledialog.askstring("Edit Expiration Date", "Enter new expiration date (YYYY-MM-DD):", initialvalue=exp_date if isinstance(exp_date, str) else exp_date_obj.strftime("%Y-%m-%d"))
            new_nutrition = simpledialog.askstring(
                "Edit Nutrition Info",
                "Enter nutrition facts (key:value, comma separated):",
                initialvalue=", ".join([f"{k}:{v}" for k, v in nutrition_info.items()])
            )
            ...
            if new_exp_date:
                try:
                    parsed_date = datetime.strptime(new_exp_date, "%Y-%m-%d")
                    if isinstance(item, dict):
                        item["expiration_date"] = new_exp_date
                    else:
                        item.expiration_date = parsed_date
                except Exception:
                    messagebox.showerror("Error", "Invalid date format. Use YYYY-MM-DD.")
            if new_nutrition is not None:
                nutrition_dict = {}
                for pair in new_nutrition.split(","):
                    if ":" in pair:
                        k, v = pair.split(":", 1)
                        nutrition_dict[k.strip()] = v.strip()
                if isinstance(item, dict):
                    item["nutrition_info"] = nutrition_dict
                else:
                    item.nutrition_info = nutrition_dict
        ...
        label_code = tk.Label(detail_frame, text="Enter Barcode Number:", font=APP_FONT, justify="center", bg=self.bg_color)
        label_code.pack(pady=5)

        self.barcode_entry = tk.Entry(detail_frame)
        ...
        nutrition_frame = tk.Frame(main_frame, bg=self.bg_color)
        nutrition_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=20, pady=20)

        tk.Label(nutrition_frame, text="Nutrition Facts:", font=APP_FONT_BOLD, bg=self.bg_color).pack(anchor="w")
        if nutrition_info:
            for key, value in nutrition_info.items():
                tk.Label(nutrition_frame, text=f"{key}: {value}", font=APP_FONT, bg=self.bg_color, anchor="w", justify="left").pack(anchor="w")
        else:
            tk.Label(nutrition_frame, text="No nutrition info available.", font=APP_FONT, bg=self.bg_color, anchor="w", justify="left").pack(anchor="w")
    ...
    def update_frame(self):
        ...
        for barcode in barcodes:
            ...
            # Only add if new barcode detected
            if getattr(self, "last_barcode", None) != barcode_data:
                ...
                # Call main app to add item
                self.root.after(0, lambda: self.add_scanned_item(barcode_data))

        # ...existing code to display frame...
        self.root.after(10, self.update_frame)

    def add_scanned_item(self, barcode_data):
        # Fetch nutrition info
        nutrition_info = self.root.fetch_open_food_facts(barcode_data)
        product_name = nutrition_info.get("Product Name", "Unknown")

        # Prompt for expiration date and category
        expiration_date = simpledialog.askstring("Expiration Date", "Enter expiration date (YYYY-MM-DD):")
        if not expiration_date:
            expiration_date = (datetime.now() + dt.timedelta(days=7)).strftime("%Y-%m-%d")  # Default: 7 days from now

        category = simpledialog.askstring("Category", "Enter category:")
        if not category:
            category = "Misc"

        # Add item to main app
        new_item = {
            "name": product_name,
        ...
        self.root.items.append(new_item)
        self.root.save_items()
        messagebox.showinfo("Item Added", f"Added: {product_name}\nBarcode: {barcode_data}")
        self.root.refresh_views()
    ...
    def fetch_open_food_facts(self, barcode, item=None):
        ...
        try:
            ...
            if data.get('status') == 1:
                ...
                # self.show_nutrition_info(nutrition_info)  # <-- REMOVE THIS LINE
    ...
    #     # Recursively update all widgets' backgrounds and foregrounds
    ...
    #     # If you use canvases, update their bg too
    #     for widget in self.root.winfo_children():
...
#            )
    ...
    def choose_bg_color(self):
        ...
        if color and color[1]:
            ...
            self.save_settings_to_file()  # Save the color for persistence
            self.apply_bg_color_to_app()
    ...
    def apply_bg_color_to_app(self):
        ...
        def update_widget_colors(widget):
            try:
                ...
                if current_bg not in item_colors:
                    widget.configure(bg=self.bg_color)
            except Exception:
                pass
            for child in widget.winfo_children():
...
#        self.camera_label.grid(row=0, column=0, columnspan=2, pady=10)
    ...
    def save_new_item(self, name, barcode, expiration_date, category):
        ...
        # Fetch nutrition info if barcode is provided
        nutrition_info = {}
        if barcode:
            nutrition_info = self.fetch_open_food_facts(barcode)
            # Optionally update name if product name found
            product_name = nutrition_info.get("Product Name", "")
            if product_name and product_name != "Unknown":
                name = product_name
        ...
        tk.Label(self.right_panel, text=name, font=APP_FONT_BOLD, bg=self.bg_color).pack(pady=(5, 2))
        tk.Label(self.right_panel, text=f"Barcode: {barcode}", font=APP_FONT, bg=self.bg_color).pack(pady=(0, 2))
        tk.Label(self.right_panel, text=f"Expires: {expiration_date}", font=APP_FONT, bg=self.bg_color).pack(pady=(0, 5))
        tk.Label(self.right_panel, text=f"Category: {category}", font=APP_FONT, bg=self.bg_color).pack(pady=(0, 5))
    ...
    def clear_screen(self):
        ...
        # Unbind mouse wheel from all canvases
        for widget in self.root.winfo_children():
            if isinstance(widget, tk.Canvas):
                ...
                if decoded_barcodes:
                    for barcode in decoded_barcodes:
                        ...
                        if getattr(self, "last_barcode", None) != barcode_data:
                            ...
                            self.add_scanned_item(barcode_data)
...
#                forecast = data['list'][index]  # 24 hours apart
    ...
    def draw_sun_banner(self, parent, sunrise_ts, sunset_ts):
        ...
        sun_canvas.place(x=10, y=10)

        def redraw_sun(event=None):
            width = sun_canvas.winfo_width()
            height = sun_canvas.winfo_height()
            sun_canvas.delete("all")

            margin = 10
            sun_radius = max(5, min(width, height) * 0.05)  # scale sun radius with canvas size

            # Draw semi-transparent sun path arc
            sun_canvas.create_arc(
                margin, margin,
                width - margin, height * 2,
                start=180, extent=180,
                style="arc",
                #outline="#FFD70080",  # yellow with transparency (alpha not supported by default in Tkinter, so using light yellow)
                outline="yellow",
                width=2
            )

            # Calculate current sun position along the semi-circle
            import math
            angle = math.pi * fraction_of_day  # 0 -> pi
            arc_radius = (width - 2 * margin) / 2
            center_x = width / 2
            center_y = height
            sun_x = center_x + arc_radius * math.cos(math.pi - angle)
            sun_y = center_y - arc_radius * math.sin(angle)
            ...
            # Draw sunrise and sunset labels
            sun_canvas.create_text(
                margin, height - 5,
                text=sunrise_str,
                anchor="sw",
                font=("Arial", int(height * 0.12)),
                fill="white"
            )
            sun_canvas.create_text(
        ...
        # Bind canvas resize to redraw function
        sun_canvas.bind("<Configure>", redraw_sun)
        redraw_sun()

    def draw_sun_banner_a(self, parent, sunrise_ts, sunset_ts):
        # Convert timestamps to datetime strings
        sunrise_dt = datetime.fromtimestamp(sunrise_ts)
        sunset_dt = datetime.fromtimestamp(sunset_ts)
        sunrise_str = sunrise_dt.strftime("%H:%M")
        ...
        sun_canvas = tk.Canvas(
            ...
            bg="skyblue",  # transparent background so background image shows
            highlightthickness=0
        )
        #sun_canvas.pack(side="left", padx=10, pady=10)
        sun_canvas.pack(anchor="nw", padx=10, pady=10)
        #sun_canvas.place(x=10, y=10)

        def redraw_graph(event=None):
            # Clear previous drawings
            sun_canvas.delete("all")

            # Dynamically adjust sizing
            w = sun_canvas.winfo_width()
            h = sun_canvas.winfo_height()
            margin = 10
            radius = (w - 2 * margin) / 2
            ...
            import math
            angle_rad = math.pi * fraction_of_day
            center_x = w / 2
            center_y = h
            sun_x = center_x + radius * math.cos(math.pi - angle_rad)
            sun_y = center_y - radius * math.sin(angle_rad)
            ...
            sun_canvas.create_text(margin, h - 5, text=sunrise_str, anchor="sw", font=("Arial", 8))
            sun_canvas.create_text(w - margin, h - 5, text=sunset_str, anchor="se", font=("Arial", 8))

        # Initial draw
        redraw_graph()
        # Redraw on resize
        sun_canvas.bind("<Configure>", redraw_graph)
    ...
    def draw_moon_calendar(self, parent):
        calendar_frame = tk.Frame(parent, bg=self.bg_color)
        ...
        cal_frame = tk.Frame(calendar_frame, bg=self.bg_color)
            ...
            for col_idx, day in enumerate(week):
                if day == 0:
                    tk.Label(cal_frame, text="", width=3, height=1,
                             bg=self.bg_color).grid(row=row_idx, column=col_idx, padx=1, pady=1)
                else:
...
class SpotifyApp:
    ...
    def create_ui(self):
        ...
        # Spotify info
        self.track_label = tk.Label(self.frame, text="Loading...", font=APP_FONT, bg=self.bg_color)
        ...
        canvas = tk.Canvas(track_container, height=220, bg=self.bg_color, highlightthickness=0)
        canvas.pack(side=tk.TOP, fill=tk.X, expand=True)

        for name, artist, image_url, link in tracks[:5]:  # show 5 tracks
            label = tk.Label(self.frame, text=f"{name} - {artist}", bg=self.bg_color, font=APP_FONT)
            label.pack()
        ...
        for name, artist, image_url, link in tracks[:5]:
            card = tk.Frame(scroll_frame, bg=self.bg_color, bd=1, relief="solid", padx=5, pady=5)
            ...
            try:
                ...
                photo = ImageTk.PhotoImage(img)
                img_label = tk.Label(card, image=photo, bg=self.bg_color, cursor="hand2")
                img_label.image = photo  # keep reference
                img_label.pack()
                img_label.bind("<Button-1>", lambda e, url=link: webbrowser.open(url))
            except Exception as e:
                print("Image load failed:", e)
                tk.Label(card, text="[Image not loaded]", bg=self.bg_color).pack()

            tk.Label(card, text=name, bg=self.bg_color, font=APP_FONT, wraplength=100).pack()
            tk.Label(card, text=artist, bg=self.bg_color, font=APP_FONT, wraplength=100).pack()
    ...
    def display_tracks(self, tracks):
        ...
        canvas = tk.Canvas(self.frame, bg=self.bg_color, height=220)
        ...
        for name, artist, image_url, link in tracks[:5]:
            card = tk.Frame(inner_frame, bg=self.bg_color, bd=1, relief="solid", padx=5, pady=5)
            card.pack(side=tk.LEFT, padx=10, pady=10)
            ...
            try:
                ...
                print("Image load failed:", e)
                tk.Label(card, text="[Image failed]", bg=self.bg_color).pack()

            tk.Label(card, text=name, font=APP_FONT, bg=self.bg_color, wraplength=100).pack()
            tk.Label(card, text=artist, font=APP_FONT, bg=self.bg_color, wraplength=100).pack()
    ...
    def display_tracks_vert(self, tracks):
        track_frame = tk.Frame(self.frame, bg=self.bg_color)
        track_frame.pack(pady=10)

        for name, artist, image_url, link in tracks[:5]:
           track_card = tk.Frame(track_frame, bg=self.bg_color)
           ...
           try:
                    ...
                    img = Image.open(BytesIO(img_data)).resize((100, 100))
                    photo = ImageTk.PhotoImage(img)
                    #img_label = tk.Label(self.frame, image=photo, cursor="hand2", bg=self.bg_color)
                    #img_label.image = photo
                    #img_label.pack()
                    #img_label.bind("<Button-1>", lambda e, url=link: webbrowser.open(url))
           except Exception as e:
                    print("Image load failed:", e)
                    fallback = tk.Label(self.frame, text="[Image not loaded]", bg=self.bg_color)
                    fallback.pack()

           if photo:
                    img_label = tk.Label(track_card, image=photo, cursor="hand2", bg=self.bg_color)
                    img_label.image = photo  # Prevent garbage collection
                    img_label.pack()
                    img_label.bind("<Button-1>", lambda e, url=link: webbrowser.open(url))

           tk.Label(track_card, text=name, font=APP_FONT, bg=self.bg_color).pack()
           tk.Label(track_card, text=artist, font=APP_FONT, bg=self.bg_color).pack()
    ...
    def display_tracks_horizontal(self, tracks):
        ...
        canvas = tk.Canvas(self.tracks_container, bg=self.bg_color, height=240)
        canvas.pack(fill=tk.X, padx=10, pady=10)
        #canvas.pack(padx=10, pady=10)

        # Canvas + Scrollbar setup
        #canvas = tk.Canvas(self.frame, bg=self.bg_color, height=240)
        #canvas.pack(fill=tk.X, padx=10, pady=10)
...
#           )
    ...
    def create_ui(self):
        ...
        # --- Spotify Section ---
        self.track_label = tk.Label(self.frame, text="Loading Spotify info...", font=APP_FONT, bg=self.bg_color)
        self.track_label.pack(pady=10)

        self.album_art_label = tk.Label(self.frame, bg=self.bg_color)
        ...
        npr_controls = tk.Frame(self.frame, bg=self.bg_color)
        ...
        # Icons side by side
#        icons_frame = tk.Frame(npr_controls, bg=self.bg_color)
#        icons_frame.pack()
...
#        npr_play_controls = tk.Frame(npr_controls, bg=self.bg_color)
...
#        podcast_btn.pack(pady=5)
        ...
        #podcast_frame = tk.Frame(self.frame, bg=self.bg_color)
        ...
        track_container = tk.Frame(self.frame, bg=self.bg_color)
        track_container.pack(fill=tk.X, padx=10, pady=(0, 10))  # Reduced top padding

        track_canvas = tk.Canvas(track_container, height=180, bg=self.bg_color, highlightthickness=0)
        ...
        scroll_frame = tk.Frame(track_canvas, bg=self.bg_color)
        ...
        tracks = self.fetch_playlist_tracks()
        for name, artist, image_url, link in tracks[:5]:
            card = tk.Frame(scroll_frame, bg=self.bg_color, bd=1, relief=tk.RIDGE)
            ...
            tk.Label(card, text=name, bg=self.bg_color, wraplength=100, font=APP_FONT_BOLD).pack()
            tk.Label(card, text=artist, bg=self.bg_color, wraplength=100, font=APP_FONT).pack()
    ...
    def display_tracks(self, tracks):
        ...
        canvas = tk.Canvas(track_container, bg=self.bg_color, height=200, highlightthickness=0)
...
#        import webbrowser
        ...
        yt_container = tk.Frame(self.frame, bg=self.bg_color)
        yt_container.pack(fill=tk.X, padx=10, pady=10)

        yt_canvas = Canvas(yt_container, height=240, bg=self.bg_color, highlightthickness=0)
        ...
        scroll_frame = tk.Frame(yt_canvas, bg=self.bg_color)
        ...
        for video in videos:
            ...
            tk.Label(card, text=video["title"], bg=self.bg_color, wraplength=120, font=APP_FONT).pack()
