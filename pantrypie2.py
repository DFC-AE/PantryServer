...
#        self.cal.pack(side=tk.LEFT, padx=10, pady=10, ipadx=20, ipady=20)
        ...
        dark_mode_btn = tk.Button(button_frame, cursor="hand2", image=lightImg, width=100, height=100, command=self.choose_bg_color)
...
#        Hovertip(back_btn, "Click to Return to the Previous Screen", hover_delay=500)
    ...
    def create_list_view(self, item=None):
        ...
        for item in self.items:
            ...
            frame = tk.Frame(scroll_frame, bg=self.bg_color)
    ...
    def show_detail_view(self, item):
        ...
        # Support both Item objects and dicts
        name = getattr(item, "name", None) or item.get("name", "")
        exp_date = getattr(item, "expiration_date", None) or item.get("expiration_date", "")
        if isinstance(item, dict):
            nutrition_info = item.get("nutrition_info", {})
        else:
            nutrition_info = getattr(item, "nutrition_info", {})
        # If expiration_date is a string, parse it
        ...
        if hasattr(item, "days_until_expired"):
            days = item.days_until_expired()
        elif exp_date_obj and isinstance(exp_date_obj, datetime):
...
#            )
    ...
    def apply_bg_color_to_app(self):
        ...
        def update_widget_colors(widget):
            try:
                # Only update if not an item color
                current_bg = widget.cget("bg")
                # List of possible item colors (from get_color)
                item_colors = [
                    "#FF0000", "#FF1A00", "#FF3300", "#FF4D00", "#FF6600", "#FF8000",
                    "#FF9900", "#FFB200", "#FFCC00", "#FFE500", "#E5FF00", "#CCFF00",
                    "#99FF00", "#66FF00", "#33FF00"
