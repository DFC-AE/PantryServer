...
#        self.weather_ui()
    ...
    def create_conversion_table_panel(self, parent):
            ...
            tk.Label(panel, text="Unit Converter", font=("Helvetica", 14, "bold"), bg="white").pack(pady=(10, 5))
    ...
    def create_conversion_table_panel_old(self, parent):
        ...
        title = tk.Label(panel, text="Unit Conversion", font=("Helvetica", 14, "bold"), bg="white")
        ...
        tk.Label(content, textvariable=self.result_var, bg="white", font=("Helvetica", 11)).grid(
...
#        recipe_title = tk.Label(recipe_frame, text="Random Recipe Idea", font=("Helvetica", 14, "bold"), bg="white")
#        recipe_title.pack(pady=(10, 5))

#        self.recipe_img_label = tk.Label(recipe_frame, bg="white")
#        self.recipe_img_label.pack(pady=5)

#        self.recipe_text = tk.Label(recipe_frame, text="Loading recipe...", wraplength=400, bg="white", font=("Helvetica", 11), justify="left")
...
#        tk.Label(converter_frame, text="Unit Converter", font=("Helvetica", 14, "bold"), bg="white").pack(pady=10)

        # Input field
        #self.convert_input = tk.Entry(converter_frame, font=("Helvetica", 12), justify="center")
        ...
        # Result label
        #self.convert_result = tk.Label(converter_frame, text="", font=("Helvetica", 12), bg="white")
    ...
    def create_expiring_soon_panel(self, parent):
        ...
        title = tk.Label(panel, text="Expiring Soon", font=("Helvetica", 14, "bold"), bg="white")
        title.pack(pady=(10, 5))
        ...
        if not expiring_items:
            tk.Label(scrollable_frame, text="No items expiring soon.",
                     font=("Helvetica", 11), bg="white", fg="gray").pack(pady=10)
        else:
    ...
    def create_random_recipe_panel(self, parent):
        ...
        title = tk.Label(panel, text="Random Recipe", font=("Helvetica", 14, "bold"), bg="white")
        title.pack(pady=(10, 5))
        ...
        # Recipe name
        self.recipe_name_label = tk.Label(panel, text="", font=("Helvetica", 12, "bold"),
    ...
    def show_full_recipe_view(self, meal):
        ...
        tk.Label(text_inner, text=meal["strMeal"], font=("Helvetica", 18, "bold"),
                 bg="white", anchor="w").pack(anchor="w", pady=(0, 5))

        tk.Label(text_inner, text=f"Category: {meal['strCategory']}   Rating: {rating:.1f} ",
                 font=("Helvetica", 12), bg="white", anchor="w").pack(anchor="w")
...
#        tk.Label(text_frame, text=meal["strMeal"], font=("Helvetica", 18, "bold"), bg="white").pack(anchor="w", pady=5)
#        tk.Label(text_frame, text=f"Category: {meal['strCategory']}   {rating}", font=("Helvetica", 12), bg="white").pack(anchor="w")
...
#            print("Error loading image:", e)
        ...
        # Meal name
        tk.Label(scrollable_frame, text=meal["strMeal"], font=("Helvetica", 18, "bold"), bg="white").pack(pady=10)
        ...
        tk.Label(scrollable_frame, text="Ingredients:", font=("Helvetica", 14, "bold"), bg="white").pack(pady=5)
        for i in range(1, 21):
            ingredient = meal.get(f"strIngredient{i}")
            measure = meal.get(f"strMeasure{i}")
            if ingredient and ingredient.strip():
                tk.Label(scrollable_frame, text=f" {ingredient} - {measure}",
                         font=("Helvetica", 12), bg="white", anchor="w", justify="left").pack(fill=tk.X, padx=20)

        # Instructions
        tk.Label(scrollable_frame, text="Instructions:", font=("Helvetica", 14, "bold"), bg="white").pack(pady=(10, 5))
        tk.Label(scrollable_frame, text=meal["strInstructions"], wraplength=700, justify="left",
                 font=("Helvetica", 12), bg="white").pack(padx=20, pady=5)
    ...
    def show_full_recipe_view_olde(self, meal):
        ...
        # Recipe title
        tk.Label(self.root, text=meal['strMeal'], font=("Helvetica", 20, "bold"), bg="white").pack(pady=10)
        ...
        # Ingredients
        tk.Label(self.root, text="Ingredients:", font=("Helvetica", 14, "bold"), bg="white").pack(pady=5)
        ...
        for i in range(1, 21):
            ...
            if ing and ing.strip():
                tk.Label(ingredients_frame, text=f"{ing} - {measure}",
                         font=("Helvetica", 12), bg="white").pack(anchor="w")

        # Instructions
        tk.Label(self.root, text="Instructions:", font=("Helvetica", 14, "bold"), bg="white").pack(pady=(10, 5))
        instructions = tk.Text(self.root, wrap=tk.WORD, height=10, font=("Helvetica", 12), bg="white")
    ...
    def show_full_recipe_view_old(self):
        ...
        # Title
        tk.Label(self.root, text=name, font=("Helvetica", 20, "bold"), bg="white").pack(pady=10)
...
#        Hovertip(back_btn, "Click to Return to the Previous Screen", hover_delay=500)
    ...
    def create_card_view(self):
        ...
        for item in self.items:
            ...
            c_btn = tk.Button(
               scroll_frame, text=text, bg=color, fg="black", font=("Helvetica", 16), wraplength=150,
    ...
    def create_list_view(self):
        ...
        for item in self.items:
            ...
            days = item.days_until_expired()
            text = f"{item.name} - Expires in {check_dates(days)} days" if days >= 0 else f"{item.name} - Expired"
            tk.Label(frame, text=text, bg=color, fg="black", font=("Helvetica", 14)).pack(side=tk.LEFT, fill=tk.X, expand=True)
    ...
    def show_detail_view(self, item):
        ...
        if item.nutrition_info:
            ...
            tk.Label(nutrition_frame, text="Nutrition Facts:", font=("Helvetica", 18, "underline"), anchor="w", justify="left").pack(anchor="w")

            for key, value in item.nutrition_info.items():
                fact = f"{key}: {value}"
                tk.Label(nutrition_frame, text=fact, font=("Helvetica", 14), anchor="w", justify="left").pack(anchor="w")


        label = tk.Label(self.root, text=detail_text, font=("Helvetica", 20), justify="left")
        label.pack(pady=30)
...
#        Hovertip(barcode_btn, "Click to Display Scanned Barcode", hover_delay=500)
        ...
        label_code = tk.Label(self.root, text="Enter Barcode Number:", font=("Helvetica", 15), justify="center")
...
#        Hovertip(manual_btn, "Click to Enter Barcode Manually", hover_delay=500)
    ...
    def show_nutrition_info(self, nutrition_info):
        ...
        for key, value in nutrition_info.items():
            label = tk.Label(info_window, text=f"{key}: {value}", font=("Helvetica", 14), anchor="w")
    ...
    def add_item_popup(self):
        ...
        tk.Label(self.root, text="Enter Item Name:", font=("Helvetica", 15)).pack(pady=5)
        ...
        tk.Label(self.root, text="Select Expiration Date:", font=("Helvetica", 15), justify="center").pack()
...
weather_label = tk.Label(root, text="Loading weather...", font=("Helvetica", 14))
...
#        content_frame.place(relx=0.5, rely=0.5, anchor="center")
        ...
        #self.weather_label = tk.Label(self.root, font=("Helvetica", 16))
        #self.weather_label = tk.Label(content_frame, font=("Helvetica", 16), bg="white")
        self.weather_label = tk.Label(content_frame, font=("Helvetica", 16))
        ...
        for _ in range(5):
            ...
            #text_label = tk.Label(day_frame, font=("Helvetica", 10), bg="white")
            text_label = tk.Label(day_frame, font=("Helvetica", 10))
            text_label.pack()
...
class SpotifyApp:
    ...
    def create_ui(self):
        ...
        # Spotify info
        self.track_label = tk.Label(self.frame, text="Loading...", font=("Helvetica", 16), bg="white")
        ...
        tk.Button(controls, text="Play", font=("Helvetica", 14), command=self.play).pack(side=tk.LEFT, padx=10)
	...
	## Requires Auth Token not Client ##
        ...
        for name, artist, image_url, link in tracks[:5]:  # show 5 tracks
            label = tk.Label(self.frame, text=f"{name} - {artist}", bg="white", font=("Helvetica", 12))
            label.pack()
    ...
    def display_tracks(self, tracks):
        ...
        for name, artist, image_url, link in tracks[:5]:
            ...
            tk.Label(card, text=name, font=("Helvetica", 10), bg="white", wraplength=100).pack()
            tk.Label(card, text=artist, font=("Helvetica", 8), bg="white", wraplength=100).pack()
    ...
    def display_tracks_vert(self, tracks):
        ...
        for name, artist, image_url, link in tracks[:5]:
           ...
           tk.Label(track_card, text=name, font=("Helvetica", 10), bg="white").pack()
           tk.Label(track_card, text=artist, font=("Helvetica", 8), bg="white").pack()
...
class MusicApp:
    ...
    def create_ui(self):
        ...
        # --- Spotify Section ---
        self.track_label = tk.Label(self.frame, text="Loading Spotify info...", font=("Helvetica", 16), bg="white")
        ...
        tk.Button(controls, text="Play",font=("Helvetica", 12), command=self.play_spotify).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(npr_controls, image=self.nprImg, font=("Helvetica", 14)).pack()
        ...
        #podcast_btn = tk.Button(podcast_frame, image=podImg, font=("Helvetica", 12), command=self.open_apple_podcast)
        #podcast_btn.pack(side=tk.LEFT, padx=10, pady=2)
        #podcast_btn = tk.Button(self.frame, image=self.podImg, font=("Helvetica", 12),
        ...
        for name, artist, image_url, link in tracks[:5]:
            ...
            tk.Label(card, text=name, bg="white", wraplength=100, font=("Helvetica", 9, "bold")).pack()
            tk.Label(card, text=artist, bg="white", wraplength=100, font=("Helvetica", 8)).pack()
    ...
    def display_youtube_videos(self):
        ...
        for video in videos:
            ...
            tk.Label(card, text=video["title"], bg="white", wraplength=120, font=("Helvetica", 9)).pack()
